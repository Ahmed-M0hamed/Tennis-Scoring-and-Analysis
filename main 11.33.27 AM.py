...
def main():
    ...
    players_racket_predictions =infernece_class.track_player_and_racket(frames  , os.path.join(os.getcwd() ,'model_checkpoints/yolo11x.pt') , presaved = False  ,
                                                                                                saved_prediction_path = os.path.join(os.getcwd() , 'saved_preds/player_racket.pkl'))
    ball_predictions = infernece_class.track_ball(frames  , os.path.join(os.getcwd() ,'model_checkpoints/best.pt') , presaved = False ,
                                                                                           saved_prediction_path = os.path.join(os.getcwd() , 'saved_preds/ball_2.pkl'))
    court_predictions = infernece_class.court_keypoints( frames  , os.path.join(os.getcwd() ,'model_checkpoints/keypoints_model.pth') , presaved = False   , 
